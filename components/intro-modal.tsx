    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed inset-0 z-[500] flex items-center justify-center bg-scrim backdrop-blur-sm p-4"
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-3xl bg-popover border border-border/60 rounded-sm shadow-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 pt-6 pb-4">
              ...
              <button
                onClick={onClose}
                className="p-1.5 rounded-sm text-muted-foreground/40 hover:text-foreground hover:bg-surface-overlay transition-colors"
                aria-label="Close"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-border/60">
              ...
              <button
                onClick={onClose}
                className="px-4 py-1.5 text-xs font-mono font-medium rounded-sm bg-surface-overlay hover:bg-surface-overlay-hover text-foreground/70 hover:text-foreground border border-border/60 hover:border-border transition-all"
              >
                Skip to app →
              </button>
            </div>
